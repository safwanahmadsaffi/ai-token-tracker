(() => {
  ...
  var PRESET_LIMITS = {
    ...
    "gpt-4o-mini": 25e4,
    "o3-mini": 5e4,
    "claude-3-5-sonnet": 45e3,
    "claude-3-5-haiku": 15e4,
    "deepseek-r1": 1e5,
    "deepseek-v3": 2e5,
  ...
  async function renderStats() {
    ...
    const dailyLimit = res?.dailyLimit || 1e5;
  ...
  async function handlePresetChange() {
    ...
    if (val === "custom") {
      ...
      const limit = PRESET_LIMITS[val] || 1e5;
  ...
  function extractConversationFromPage() {
    ...
    const SELECTORS = {
      ...
      deepseek: {
        turn: '[class*="message"]',
        role: (el) => el.className.toLowerCase().includes("user") ? "user" : "assistant"
      },
      gemini: {
  ...
  function conversationToMarkdown(convo) {
    ...
    convo.turns.forEach((t) => {
      lines.push(`### ${t.role === "user" ? "\u{1F9D1} User" : "\u{1F916} Assistant"}`);
      lines.push("");
      lines.push(t.text);
      lines.push("");
    });
  ...
  async function detectActivePlatformBadge() {
    try {
      ...
      document.getElementById("platformBadge").textContent = found ? `Active on ${found[1]}` : "Not on a supported LLM site";
    } catch {
    }
  }
  var presetSelectEl = document.getElementById("presetQuotaSelect");
  if (presetSelectEl) {
